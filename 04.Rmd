...
```{r filt,include=T}
sg(filt[step!='sampled'][.N,step:='100 Percent'] %>% {.[,lapply(.SD,function(x) c(as.character(x[-.N]),x[.N] %>% nn))]} %>%  setnames(ec('Step,Document,Page,Paragraph,Sentence,Token,Term,Lemma')),tit='Term filtering due to data management',digit.separator=',',font.size='small')
```
