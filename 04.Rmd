...
warp<-function(gg,r) gg+geom_rect(data=war,aes(xmin=xmin,xmax=xmax,ymin=r[1],ymax=r[2]),fill=war$col,alpha=.1,inherit.aes = F) +
    geom_text(data=war,aes(x=x,y=war$dodge*r[2],label=text),color=war$col,inherit.aes = F)
emp<-data.table(
           ...
           ) %>% melt(id.vars='Date',variable.name='Sector',value.name='Percent Labor Market')
plt(warp(myth(ggplot(data=emp
         ,aes(x=Date,y=`Percent Labor Market`,color=Sector))) + geom_line() + geom_point() + theme_bw() 
      ,r=emp[,range(`Percent Labor Market`) %>% replace(1,0)]))
```
...
```{r lyn,include=T,fig.cap='JSTOR journal birth and death rates'}
load('d/b/pli.RData')
pli
```
