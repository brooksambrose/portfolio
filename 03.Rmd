...
p<-stm$meta[,{
  ...
  gp<-ggplot(mapping=aes(k3=k3,tit=tit,auths=auths,year=year,jour=solong,ix=ix))+
...
},.SDcols=ec('k3,edn,gini,solong,url,auths,tit,year,soci,ix')]
p
```
...
Limiting ourselves to the social science subsample, let's compare the most highly ranked music article, `r whwh(stm$meta,6249)`, with an article that is only estimated to be 50% about music, e.g. `r whwh(stm$meta,4752)`.
...
```{r, include=T}
g<-dc.f(4752,mod[[9]],trm)
noquote(g['music'])
```


```{r,include=T,fig.cap='Comparison of an article that is nearly all music and one that is half music.'}
cm<-list({ix<-6249
g<-(mod[[9]]$doc.top.theta[ix,]*mod[[9]]$top.word.phi.beta)[,mod[[9]]$documents[[ix]][1,]] 
n<-which(apply(g,2,which.max)==6) %>% names
g %<>% melt %>% data.table
