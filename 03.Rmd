...
```{r ttsgnr,include=T,fig.scap='Absolute count of term "genre", 1901-2008. Segments correspond to significantly different second derivatives.'}
...
```{r idi-hist, include=T,fig.scap='Distribution of idiosyncracy, the proportion of document vocabularly dropped during pre-processing. Pluses indicate outliers.'}
...
```{r gp-hist, include=T,fig.scap='Distribution of $log_{10}$ of the count of the term "genre" as a proportion of all terms in a text. Pluses indicate outliers.'}
...
```{r ghost, include=F,fig.scap='Sum of document proportion of terms predicted to be present but are actually mssing. Pluses indicate outliers.'}
...
```{r ghostN,include=T,fig.scap='Ghost probabilities, the sum of document proportion of terms predicted to be present but are actually missing, by document length. Ghost probabilities are on log-odds scale, and document length is on $log_{10}$ scale. The blue line is a linear fit on transformed data.',dev='png'}
...
```{r elbow1p,include=T,fig.scap='Cumulative distribution of within topic term probabilities. Dotted line at p = 2/3'}
...
```{r log10-betap,include=T,fig.scap='As K increases, curve flattens toward more higher and more lower term probabilities, logit scale. Left tail truncated at p < 1e-10.'}
...
```{r elbow2,include=T,fig.scap='Cumulative distribution of within topic document probabilities, truncated at junk threshhold. Dashed line is linear fit to endpoints.'}
h<-c(1:10)
...
```{r top-gini,include=T,fig.scap='Topic concentrations (TC) within documents by TC within terms. Diamonds proportional to term frequency explained by each topic'}
...
```{r man-con,include=T,fig.scap='Topic confusion network. Tied topics contained at least two errors in blind manual sorting test. Untied topics were perfectly separated. Colors represent a topic\'s number of imperfect tests.'}
xg<-xel[,{
...
```{r lineage,include=T,fig.asp=1,fig.scap='Top 40 term rank changes over topic 2 lineage. Blue are increasing, red decreasing, gray unchanged, and green solitary.'}
...
```{r genr-mod-viz,include=T,fig.scap='Topic Term Explorer, K=10. [*Interactive pop-out.*](ldaviz/viz/index.html#topic=6&lambda=0.5&term=){target="_blank"}'}
...
Limiting ourselves to the social science subsample, let's compare the most highly ranked music article, `r whwh(stm$meta,6249)`, with an article that is only estimated to be `r mod[[9]]$doc.top.theta[6249,6]` about music, e.g. `r whwh(stm$meta,4895)`.
...
```{r,include=T,fig.asp=1.5,fig.cap='Comparison of an article that is nearly all music and one that is half music.'}
gtw<-function(ix,tp){
...
cm[qs,on='t',g:=factor(g,levels=ec('y,m,n'))]
myth(ggplot(data=cm[!is.na(g)],aes(x=g,y=p,label=t,color=ix)) + geom_text(hjust=cm[!is.na(g)]$ix %>% as.integer %>% {ec('left,right')[.]},check_overlap = T,nudge_x = cm[!is.na(g)]$ix %>% as.integer %>% {c(.01,-.01)[.]}) + scale_y_continuous(trans='log'))
```
