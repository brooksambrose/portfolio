...
```{r mod-proj,include=T,cache=T,dev='png',fig.width=3,fig.height=3,out.width='33%',fig.show = 'hold',fig.cap='Mode Conversion on a 300 Edge Random Sample of the JSTOR Title List Label Network'}
bup<-jclu2bup.f(jclu)
```
...
if(file.exists(f)){
  load(f)
  kcc2isl<-kcc2isl.f(bel2mel,cos2kcc,wok2dbl,dbl2bel,co=kcc2isl,ordinal=F)
} else {
  kcc2isl<-kcc2isl.f(bel2mel,cos2kcc,wok2dbl,ordinal=F,rad=.8,nit=5e4)
  save(kcc2isl,file=f)
}
rm(f)
```

```{r isl3d, fig.cap='Time to Explore!', warning=FALSE, include=T}
f<-'d/b/isl3d.RData'
if(file.exists(f)){
  load(f)
} else {
  library(plotly)
  bgc<-desat(topo.colors(1),.4)
  ...
  mrk<-list(symbol='circle',size=4,opacity=1,line=list(width=0))
  cm<-ki$origData() %>% names %>% setdiff(ec('x,y,names,v,k,g,text,gn'))
  names(cm)<-cm
  #viridis::viridis
  #colorRampPalette(desat(ec('yellow,red'),.8))
  cm<-lapply(cm,function(x) {x<-ki$origData()[[x]];w<-is.na(x);y<-x[!w] %>% {colorRamps::blue2red({diff(round(range(.)))+1})[.-min(.)+1]};x<-rep(NA_character_,length(x));x[!w]<-y;x})
  adm<-lapply(cm,function(x) function(p,vis,y=x) {add_markers(p,data=ki,visible=vis,color=I(y),text=~text,hoverinfo='text',hoverlabel=list(bgcolor=I(y)),showlegend=F,marker=mrk)})
  isl3d<-plot_ly(x=~x,y=~y,z=~k+1) %>%
    add_mesh(inherit=F,data=kcc2isl,x=~x,y=~y,z=~k,intensity=~k,colors=attr(kcc2isl,'colors'),hoverinfo='skip') %>%
    adm$cc(p=.,vis=T) %>% adm$`AM ANTHROPOL`(p=.,vis=F) %>% adm$`AM J SOCIOL`(p=.,vis=F) %>% adm$py(p=.,vis=F) %>% adm$ca(p=.,vis=F) %>%
    layout(paper_bgcolor=bgc,plot_bgcolor=attr(kcc2isl,'colors')[1],dragmode='turntable',scene=list(aspectratio = list(x = 1, y = 1, z = 0.25),xaxis=bck,yaxis=bck,zaxis=bck,camera=list(eye=list(x=0,y=-.5,z=1)))
           ,updatemenus = list(list(buttons = list(
             list(method = "restyle", args = list("visible", list(T,T,F,F,F,F)),label = "Citation Count")
             ,list(method = "restyle",args = list("visible", list(T,F,T,F,F,F)),label = "Anthropology")
             ,list(method = "restyle",args = list("visible", list(T,F,F,T,F,F)),label = "Sociology")
             ,list(method = "restyle",args = list("visible", list(T,F,F,F,T,F)),label = "Avg Publication Year")
             ,list(method = "restyle",args = list("visible", list(T,F,F,F,F,T)),label = "Avg Citation Age")
           )))
    ) %>% 
    highlight(on='plotly_hover',opacityDim = .1)
  save(isl3d,file=f)
}
rm(f)
isl3d
