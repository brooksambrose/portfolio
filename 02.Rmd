...
rm(f)
jN<-jstorm[,title_history %>% unique %>% length]
```
...
if(file.exists(f)){
  ...
  jclu<-jstorm2jclu.f(jstorm) # TODO consider two passes, one identifying sups, the second removing them
  save(jclu,file=f)
}
rm(f)
```

We rely on the JSTOR digital archive which gives access to optical scans of historical journals. JSTOR provides a title list of their journal coverage [@2018JSTOR]. The coverage of journals in the archive is very complete for those journals chosen for the database. As of this writing JSTOR contained `r jstorm[,nn(.N)]` different journal titles and `r nn(jN)` journals from `r jstorm[,publisher_name %>% unique %>% length] %>% nn` different publishers. The different journal counts are due to some journals changing titles at least once.^[To avoid overcounting, title histories are collapsed into their most recent record, meaning all subsequent counts are out of `r nn(jN)`. Even though we might expect disciplinary identity to change over time, JSTOR discipline labels do not vary within title histories. `r {dN<-jstorm[title_id==title_history,sapply(discipline,length)]==0;if(any(dN)) ftn<-paste0(nn(sum(dN),cap=T),' journals--',jstorm[title_id==title_history][dN,paste(publication_title,collapse=', %%') %>% sub('(.+), %','\\1, and ',.)],'--lacked any discipline labels and are excluded from tabulations.');if(sum(dN)==2) sub(', and %',' and ',ftn) %>% gsub('%','',.) else ftn %>% gsub('%','',.)}`] <!--TODO add sentence concatenator to tilit--> These journals are organized into `r jclu$tab[,nn(.N)]` superdisciplines under which may be found `r nn(jstorm[,discipline %>% unlist %>% unique %>% length]-jclu$tab[,.N])` disciplines.

Most journals are given more than one discipline label, and the superdisciplines are not marked as such in the database  creating some redundancy. For instance, a journal labeled as "Sociology" will also be labeled as "Social Sciences". In order to separate the hierarchy in a reproducible way, I projected the journals as a bimodal network with journals in the first mode bearing ties to discipline labels in the second mode. I then projected the bimodal network into two unimodal networks, one where journals are connected by ties equal to the number of discipline labels they have in common, and another where labels are tied by the number of articles carrying both labels.

These two networks capture interdisciplinarity as defined by JSTOR coders. To draw out their disciplinary scheme I conducted community detection, a network method that classifies nodes according to the dense subnetworks they occupy, to cluster each network according to the potentially latent patterns in JSTOR's labeling policy. The goal was to confirm the expectation of which labels counted as superdisciplines in a data driven way, or to learn a different classification scheme if the results were surprising. The results in Table \@ref(tab:jclu-tab-sup) are achieved by separating the *journal-label-journal* network into the detected clusters, and then by assigning it the name of its most frequent label. The learned labels refine the superdisciplines by making them mutually exclusive, as is apparent by the sum of the given labels exceeding 100%.
...
The cluster labels comport with a commonsensical understanding of superdisciplines. Social science journals appear to be overrepresented due to JSTOR's initial focus in that area, even moreso if one adds business and economics to its total. The smallest fields are professional, in which we include business and economics which as a label conflates academic and professional journals.

Clustering of the *label-journal-label* network reveals a simplifie. Because there are far fewer labels than journals, the clustering is less granular. Figure \@ref(fig:jclu-lnet) shows a graph of the relationships among labels, where again the strength of ties is equal to the number of journals bearing both labels. Again community detection partitions the network into its most dense subregions. Here the the label with highest degree is assigned to the category and then ommited as a node. Astronomy, an isolate that never accompanies another label, is also ommited from the graph, resulting in four superdiscipline categories.
...
```{r jclu-lnet,include=T,fig.cap='Discipline Network (Latent Positions)'}
