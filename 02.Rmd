# `r {load('_init.RData');kopi();tdis}` {#dis}
...
```{r jclu-lnet,include=T,fig.cap='Discipline Network in Latent Space.'}
#fig.cap='Discipline Network in Latent Space. Node size represents sociality. The larger a node, the more attractive it is, and the larger a white dot within a node, the more repulsive it is.',
...
(ref:layouts) Fruchterman Reingold and latent space layouts compared. [*Interactive pop-out.*](`r if(latex|docx) 'https://brooksambrose.github.io/portfolio/'`exh/layouts.html){target="_blank"}

```{r layouts,include=T,fig.cap='(ref:layouts)'}
f<-'d/b/pp.RData'
if(file.exists(f)){
  load(f)
} else {
...
htmlwidgets::saveWidget(pp,file = getwd() %>% paste('exh','layouts.html',sep=.Platform$file.sep),background='white',selfcontained = F)
...
jclum[,rs:=round(rs,2)]
jclum<-jclum[!'Medicine & Allied Health',on='m']
jclum['Management & Organizational Behavior',on='n',n:='Management & Org. Behavior']
jclum['Architecture & Architectural History',on='n',n:='Architecture & Arch. History']
...
sg(jclum,tit='Disciplines by sociality multiplier',font.size='small'
