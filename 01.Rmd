...
jstorm2tab.f(jstorm,beg.bef = 1900,end.aft = 2000,caption='20th Century Social Science Journals in JSTOR')
```
...
```{r ftx2des,include=T}
des.com<-ftx2des.f(pre)
des.stm<-ftx2des.f(pre[!is.na(stm)])
knitr::kable(list(des.com,des.stm,data.table(tot=round(des.stm$cnt/des.com$cnt*100,2))),caption = 'Full Text Descriptives')
```
...
psim<-function(x) replicate(1000,rpois(mlk[x,.N],fit[x,lam]) %>% table %>% prop.table %>% data.table,simplify = F) %>% rbindlist %>% setnames(c('c','d')) %>% data.table(level=x,.)
sim<-pbapply::pblapply(fit$level %>% levels,psim) %>% rbindlist
sim[,c:=as.numeric(c)]

bw<-1
bh<-.002
p<-ggplot(mlk,aes(K,..density..,fill=level)) +   
  geom_bin2d(data=sim,aes(x=c,y=d,group=level,fill=level,alpha=..count..),binwidth=c(bw,bh),position=position_nudge(y=1e-5)) +
  geom_step(aes(x=x,y=dp,color=level),data=dp,size=1,direction = 'vh') +
# stat_bin(aes(color=level),geom="point",binwidth=bw,position=position_nudge(y=-1e-10),color='white',shape = 21,size=3) +
  stat_bin(aes(color=level,label=ifelse(..density..>0,x,'')),geom="text",binwidth=bw,size=3) +
  geom_label(aes(mK,y=1e-10,label=mK),mlk[,.(mK=median(K) %>% round(1)),by=level],color='white') +
  theme_bw() + scale_color_brewer(type = 'qual',palette='Dark2') +
...
p<-ggplot(mlk2,aes(x=count,fill=level)) +
...
geom_text(aes(y=observed,label=count,color=level),size=3) +
geom_label(aes(mK,y=1e-10,label=mK),mlk[,.(mK=median(K) %>% round(1)),by=level],color='white') +
theme_bw() + scale_color_brewer(type = 'qual',palette='Dark2') +
scale_fill_brewer(type = 'qual',palette='Dark2') + theme(legend.position = c(.9,.5))
p
...
```{r}
k<-mlk[,.(k=replicate(1000,kurtosi(K %>% sample(replace = T)))),by=level]
k[,.(e=mean(k),se=sd(k),l99=quantile(k,.005),u99=quantile(k,.995)),by=level] %>% print(digits=3)


merge(t,mlk2,by.x=c('level','c'),by.y = c('level','count'))[,.(t=(observed-fitted)/se),keyby=.(level,c)] %>% qplot(data=.,geom='smooth',mapping=aes(x=c,y=t,color=level))
