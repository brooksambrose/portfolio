...
if(docx) knitr::include_graphics('img/frex.png') else sg(frex,tit = 'Content variation across frequency and exclusivity',new.col.align = 'lll' )
```
...
For now, we will focus on the consequences of the choice of the number of topics K. We fit nine models in sequence from K = 2 to K = 10.
...
Having fit nine models sequentially from K = 2 to K = 10, we alight on the final as the focal model given that we assume that at ten topics we have still underspecified K. In this section I implement several approaches to validating topic quality mentioned above.

```{r check-residual}
if(F) ckr<-pbapply::pbapply(mod,function(x) stm::checkResiduals(x$model,x$documents))
```

<!--The largest beta values are very small.-->
...
lgth<-rbindlist(lgth)[,K:=factor(K)][,logith:=th %>% boot::logit(.)]
myth(ggplot(data=lgth,aes(x=logith,color=K)) + geom_density()) + viridis::scale_color_viridis(discrete = T,end = .9) + xlab('logit(theta)') + geom_vline(xintercept = boot::logit(tst),color='red')
```
...
Among the 45 different sorting tasks only two outcomes were observed. `r table(xel$p)[1]` tasks was performed perfectly (p = `r min(xel$p) %>% round(4)`), while in `r table(xel$p)[2]` tasks or one error was made, which is to say two documents were misclassified (p = `r max(xel$p) %>% round(4)`). Figure \@ref(fig:man-con) visualizes the pairwise outcomes as a confusion network. Topics that are disconnected were sorted perfectly, while topics that are connected were confused. The graph reveals some variation in topic confusion, as `r dxg[1] %>% nn` were confused twice, `r dxg[2] %>% nn` were confused once, and the remaining `r dxg[3] %>% nn` were never confused.
...
Models and software for hierarchical models have been developed [@Teh2006Hierarchical; @Roberts2015pkg], but they are not yet in common use by social scientists. Here
...
plt(p1) %>% plotly::style(visible="legendonly", traces = c(1,3,4,7,8,9))
```
...
```{r prob-anc}
aw<-mod[[9]]$top.word.phi.beta %>% apply(2,ineq::Gini) %>% order(decreasing = T) %>% head(1000) %>% {data.table(w=mod[[9]]$vocab[.],k=apply(mod[[9]]$top.word.phi.beta[,.],2,which.max))} %>% .[,.(w=list(w %>% head(6))),keyby=k]
aw[,p:=mod[[9]]$vocab[mod[[9]]$top.word.phi.beta %>% apply(1,which.max) %>% head(10)]]
