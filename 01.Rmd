...
p<-stm$meta[,{
  ...
  rmeta<-data.table(.SD)[,`:=`(
    ...
    ,solong=stringr::str_wrap(solong,50,exdent = 5) #][,`Soci[a-z]*`:=soci][,edn:=factor(edn,levels=htt$Cluster)
    ,`Soci[a-z]*`=soci
    ,edn=factor(edn,levels=htt$Cluster)
  ...
  gp<-ggplot(mapping=aes(k3=k3,tit=tit,auths=auths,year=year,jour=solong,ix=ix))+
    ...
    theme_bw() + scale_color_brewer(palette='Dark2') + scale_fill_manual(values=c(spine='lightgray',mixed='white')) + scale_y_reverse() + coord_flip() + xlab('Within document topic concentration (Gini)') + ylab('Topic cluster')
