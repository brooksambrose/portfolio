...
```{r genre-goog,include=T,fig.cap='Wordcloud of third term in 3gram beginning with "genre of".',screenshot.opts=list(delay = 5)}
gnrgoog[,wordcloud2::wordcloud2(.SD,widgetsize = NULL,size = .85,gridSize = 1,fontWeight = 'normal',rotateRatio = 0,color = viridis::viridis(max(n),direction = -1,option = 'E',end = .9)[n],shape='circle',shuffle = F,fontFamily = 'Times'),.SDcols=ec('com,n')]
```
...
data.frame(t=mod[[9]]$top.word.phi.beta %>% apply(1,ineq::Gini),d=mod[[9]]$doc.top.theta %>% apply(2,ineq::Gini)) %>% {myth(ggplot(data=.,aes(x=d,y=t))) +
    ...
    geom_point(size=sapply(mod[9],function(x) apply(t(x$top.word.phi.beta) * x$term.frequency,2,sum)) %>% lintran(c(5,20)),shape=1,color=rep('brown',10) %>% replace(c(1,4,5,6,10),'purple'),stroke=2) + scale_size_area() +
    geom_text(label=1:nrow(.)) 
  }
```


```{r elbow,include=T,fig.cap='Cumulative distribution of within topic term probabilities.'}
h<-c(3,8,9,10)
elb<-mod[[9]]$top.word.phi.beta[h,] %>% apply(1,sort,decreasing=T) %>% data.table %>% as.list
for(i in 1:length(h)) elb[[i]]<-data.table(t=elb[[i]][1:200] %>% cumsum,T=(h)[i])[,i:=.I]
elb<-rbindlist(elb)[,T:=factor(T)] %>% setkey(T)
myth(ggplot(elb,aes(x=i,y=t,color=T))) + geom_line(size=1) + theme(legend.position = c(.98,.02),legend.justification = ec('right,bottom'))
```
...
```{r genr-mod-viz,include=T,fig.cap='Topic Term Explorer, K=10', out.width='100%',screenshot.opts=list(delay = 2),}
