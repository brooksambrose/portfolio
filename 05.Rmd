# `r {load('_init.RData');kopi();tcit}` {#cit}
...
(ref:kcc2tree) K-clique Community Structure. [*Interactive pop-out.*](`r if(latex|docx) 'https://brooksambrose.github.io/portfolio/'`exh/kcc2tree.html){target="_blank"}

```{r kcc2tree, fig.cap='(ref:kcc2tree)',eval=T,include=T, message=FALSE, warning=FALSE}
f<-'d/b/kcc2tree.RData'
if(file.exists(f)){
  load(f)
} else {
  #{load('d/p/cos2kcc.RData');load('d/p/bel2mel.RData');load('d/p/wok2dbl.RData')}
  system.time(kcc2tree<-kcc2tree.f(cos2kcc,bel2mel,wok2dbl,sides = 100,transform=F))
  save(kcc2tree,file=f)
htmlwidgets::saveWidget(kcc2tree$int,file = getwd() %>% paste('exh','kcc2tree.html',sep=.Platform$file.sep),background='gray',selfcontained = F)
}
rm(f)
knitr::include_graphics('img/kcc2tree.png') 
#kcc2tree$int
```
...
(ref:kcc2isl) K-clique Community Island Plot. [*Interactive pop-out.*](`r if(latex|docx) 'https://brooksambrose.github.io/portfolio/'`exh/isl3d.html){target="_blank"}

```{r kcc2isl,fig.cap='(ref:kcc2isl)',include=T,eval=T,cache=T}
f<-'d/b/kcc2isl.RData'
if(file.exists(f)){
  load(f)
