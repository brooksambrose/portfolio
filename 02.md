...
\caption{Fractal Evaluation and Commensurated Scale. Adapted from Abbott (2011:11).}(\#fig:fractalp)
\end{figure}
...
\caption{Mode Conversion on a 300 Edge Random Sample of the JSTOR Title List Label Network}(\#fig:mod-proj)
\end{figure}
...
\caption{Discipline Network in Latent Space.}(\#fig:jclu-lnet)
\end{figure}
...
(ref:layouts) Fruchterman Reingold and latent space layouts compared. [*Interactive pop-out.*](https://brooksambrose.github.io/portfolio/exh/layouts.html){target="_blank"}
...
\caption{(ref:layouts)}(\#fig:layouts)
\end{figure}
